// vector3d.h - 2D/3D/4D vector and matrix math (float + 4.12 fixed-point)
//
// Copied verbatim from Oscar64's own include/gfx/vector3d.h
// (github.com/drmortalwombat/oscar64, local install: ~/oscar64/include/gfx/).
// Not adapted: this is pure platform-agnostic math with no bitmap/pixel
// format assumptions, so it needs no Oric-specific porting -- it works
// under this project's -dNOFLOAT build (that flag only disables printf/
// scanf's %f support, not float arithmetic itself; see include/crt_math.c).
